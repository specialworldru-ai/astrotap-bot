...
async def load_bans():
    ...
    async with aiosqlite.connect("users.db") as db:
        ...
        for r in await cursor.fetchall(): banned[str(r[0])] = r[1]
    return banned
...
async def ref_info(msg: types.Message):
    ...
    if row:
        ...
        await msg.answer(f"👥 *РЕФЕРАЛЫ*\n🔗 `{get_ref_link(user_id)}`\n💎 Бонус: {bonus}\n💸 Комиссия: {commission}%\n👥 Друзей: {ref_count}\n💰 Доход: {ref_income}\n{'🌟 Premium — x2!' if is_premium else '💡 Купи Premium — x2!'}", parse_mode="Markdown")
    ...
    try:
        ...
        async with aiosqlite.connect("users.db") as db:
            ...
            if ref_row and ref_row[0] and ref_row[0] != user_id:
                ...
                if commission > 0: await db.execute("UPDATE users SET ref_income = ref_income + ?, balance = balance + ? WHERE user_id = ?", (commission, commission, ref_row[0]))
            await db.commit()
    except: pass
...
async def banlist_cmd(msg: types.Message):
    ...
    await msg.answer("🚫 *БАНЫ:*\n\n" + "\n".join([f"🆔 {uid} | {r}" for uid, r in banned_users.items()]), parse_mode="Markdown")
    ...
    try:
        ...
        if balance is not None:
            async with aiosqlite.connect("users.db") as db:
                ...
                if upgrades: await db.execute("INSERT OR REPLACE INTO users (user_id, balance, username, upgrades_json) VALUES (?, ?, ?, ?)", (user_id, balance, username, json.dumps(upgrades)))
                else: await db.execute("INSERT OR REPLACE INTO users (user_id, balance, username) VALUES (?, ?, ?)", (user_id, balance, username))
...
async def main():
    global banned_users
    await init_db(); banned_users = await load_bans()
    asyncio.create_task(dp.start_polling(bot))
    await uvicorn.Server(uvicorn.Config(api, host="0.0.0.0", port=8000, log_level="info")).serve()
